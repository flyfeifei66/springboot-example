package com.zhaojufei.bizline.example.facade.client;
...
public interface MessageCenterFeign {
	 ...
	 * @return
	 */
	@PostMapping(value = "/api/email/sendEmail", produces = {
			MediaType.APPLICATION_JSON_UTF8_VALUE}, consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
	ResponseData sendEmail(@RequestHeader(org.springframework.http.HttpHeaders.AUTHORIZATION) String jwt,
			@RequestParam("emailId") String emailId, @RequestParam("receivers") List<String> receivers,
			@RequestParam("subject") String subject, @RequestParam("isHtml") Boolean isHtml,
			@RequestParam("mailContent") String mailContent, @RequestPart MultipartFile[] attachmentFiles);
}
